...
Tất cả commit bên dưới đã được push. Commit tính năng gần nhất là `e717eff`; tài liệu này được cập nhật cùng từng lát cắt hoàn chỉnh.
...
| Issues & cycles         | Danh sách Issues, tạo/sửa trường cơ bản, filter options, My issues, labels CRUD, cycles, saved views, subscriptions, due dates, archive và command palette dùng dữ liệu thật. Cycle detail gốc đã link/unlink Document bằng migration/API thật; chỉ Document workspace dùng chung hoặc cùng team mới được phép gán. Issue “Add link…” và phần Related issues giờ lưu quan hệ Issue–Issue thật, yêu cầu quyền với cả hai Issue, ghi Activity trên cả hai và chống self/duplicate link. Label selector trong Issue properties đã gán/bỏ Label qua API thật. Sub-issues hiện là hierarchy parent/child persisted, chỉ trong cùng team, list/create từ Issue detail gốc và ghi parent Activity. Reactions dùng `issue_reactions` persisted theo từng user/emoji, API GET/POST/DELETE có authorization Issue và popover/đếm reaction ở vị trí UI Circle gốc. Composer comment gốc đã upload attachment vào comment vừa được tạo, Comments API trả danh sách attachment thật và card có link download qua authorization hiện có. Header Saved View chỉ hiện Delete cho creator, API kiểm tra ownership và redirect sau success; Issue Saved View cũng group theo workspace status API, không còn status mock. Issue detail, properties, assignee, status/priority, comments/activity/subscribe, Issue attachment và context-menu actions đã live trong Docker. Status/priority/assignee/label/project/reaction/comment attachment chỉ đổi UI sau API success; khi thiếu workspace hoặc API lỗi không tạo local-only state. Thao tác authenticated end-to-end còn cần xác minh thủ công. |
| Initiatives & documents | Initiative list/detail/create/edit/archive/link/unlink project; team documents đã live. Nút remove trong tab Projects chỉ gọi API DELETE thật rồi reload, lỗi giữ nguyên list và hiện message. Edit dialog cập nhật property thường dùng; description/icon/target date có thể xoá bằng `null` persisted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- `7c7db0b`: Bổ sung migration `20260824150000_issue_reactions`, reaction per-user/per-emoji persisted và API GET/POST/DELETE có authorization Issue. Nút SmilePlus/popover và count chips trong Issue detail gốc chỉ cập nhật sau API success. Docker API/web production build pass; migration/table cùng health API/login route HTTP 200 đã xác minh. Acceptance đã đăng nhập còn chờ xác minh.
- `195fa86`: Nối icon Plus trong composer comment gốc vào Attachment API cho entity `comment`; tạo comment rồi upload file tối đa 10 MB, render link download từ Comments API và giữ lỗi upload rõ ràng nếu comment đã lưu nhưng file thất bại. Không cần migration vì Attachment đã có polymorphic entity contract và authorization team. Docker API/web production build pass; health/login HTTP 200 và protected comments/attachments routes trả HTTP 401 khi chưa đăng nhập. Acceptance đã đăng nhập còn chờ xác minh.
- `73b6790`: Loại bỏ Agent chat và Reviews/PR mock khỏi toàn bộ route trực tiếp (`/agent`, `/reviews`, `/review/...`). Sidebar và MainLayout gốc được giữ nguyên; route hiện thông báo unavailable thay vì render dữ liệu/chat giả. Web production Docker build/recreate và login route HTTP 200 pass.
- `1998d53`: Nối nút remove tối thiểu ở tab Initiative Projects vào API `DELETE /initiatives/:initiativeId/projects/:projectId`; không có optimistic removal, chỉ reload list sau success và lỗi hiện tại chỗ. Web production Docker build/recreate và login route HTTP 200 pass. Acceptance đã đăng nhập còn chờ xác minh.
- `4eeeae9`: Hoàn thiện Edit/Archive Initiative từ UI detail gốc qua API protected `PATCH`/`DELETE`; form cập nhật name/description/status/priority/health/icon/target date và chỉ reload/redirect sau success. Contract backend nay cho phép clear description, icon và target date thật bằng `null`, không giữ dữ liệu cũ. Docker API/web build/recreate pass; health/login HTTP 200 và PATCH chưa đăng nhập trả 401. Acceptance đã đăng nhập còn chờ xác minh.
- `5c56be8`: Nối menu More ở Saved View header vào DELETE API thật; chỉ creator thấy Delete, server vẫn kiểm tra ownership, lỗi giữ trang hiện tại và success redirect về Views. Web production Docker build/recreate và login route HTTP 200 pass. Acceptance đã đăng nhập còn chờ xác minh.
- `0800da7`: Member Profile không còn import `displayOrderedStatus` mock. Assigned/Created giữ nguyên layout Circle nhưng lấy các cột status (tên, màu, category/icon) từ `GET /issues/options` qua Issue store; web production Docker build/recreate và login route HTTP 200 pass. Acceptance đã đăng nhập còn chờ xác minh.
- `e717eff`: Issue Saved View không còn import `displayOrderedStatus` mock. UI filter/group gốc giữ nguyên nhưng các cột status lấy từ `GET /issues/options` qua Issue store; web production Docker build/recreate và login route HTTP 200 pass. Acceptance đã đăng nhập còn chờ xác minh.
...
15.   Trong comment composer gốc, tạo comment kèm file nhỏ, refresh rồi download file để xác minh attachment thuộc đúng comment; thử file lớn hơn 10 MB và xác nhận client chặn. Khi upload lỗi sau tạo comment, comment phải vẫn thấy được cùng message lỗi rõ ràng.
16.   Không mở rộng sang PR link trong lần xác minh này.
...
2. **Issue detail còn lại** — PR links chỉ làm khi có schema/API/permission đầy đủ. Nếu chưa có thì giữ unavailable.
