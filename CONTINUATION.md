...
Tất cả commit bên dưới đã được push. Commit tài liệu gần nhất là `35e9099`; commit tính năng gần nhất là `f7994e1`.
...
### Việc cần làm trước khi tiếp tục: runtime verification Attachment, Due date, Context menu, Command palette và Sidebar

UI đích: `apps/web/components/common/issues/details/issue-details.tsx`.

Paperclip ở Issue detail đã được nối vào Attachment API tại `8c6bbd8`, due date đã được nối vào `PATCH /issues` tại `603c994`, context menu đã nối subscription/archive tại `ca19039`, command palette đã nối API workspace/cycle tại `74e21c7`, và sidebar đã bỏ mock records tại `f7994e1`. Mã nguồn, API tests và production web build đã được xác minh, nhưng container `api`/`web` đang chạy chưa chứa thay đổi vì chưa được phép rebuild Docker. Agent tiếp theo cần:
...
7. Ghi rõ kết quả runtime vào hai tài liệu này rồi commit/push documentation.
8. Không mở rộng sang comment attachment, reaction, sub-issue hay relation trong lần xác minh này.
