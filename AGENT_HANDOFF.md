...
| Projects | Implemented baseline | Original project list, creation, issue progress, overview, issues, activity, timeline peek, and project header use API data. Project-list lead/priority/status/target-date controls now persist through the Projects API and record activity; workspace members are the only selectable leads. Timeline peek's unsupported controls are disabled instead of mock. Health popover has persisted health only; project subscription and project update writes are explicitly unavailable. Project settings still need their final audit. |
...
| Project Health popover | `migrated` / `unavailable` | Displays the persisted health field; Subscribe and New update remain disabled until a project subscription/update API exists. |
