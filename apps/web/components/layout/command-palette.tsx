...
export function CommandPalette() {
   ...
   const branchName = issue
      ? `${(members[0]?.name ?? 'user').toLowerCase()}/${issue.identifier.toLowerCase()}-${issue.title
           .toLowerCase()
           .replace(/[^a-z0-9]+/g, '-')
           .replace(/^-|-$/g, '')
           .slice(0, 40)}`
                  ...
                  {route === 'root' && issue && (
                     <>
                        <CommandGroup heading="Issue">
                           ...
                           <CommandItem
                              onSelect={() => {
                                 void updateIssueAssignee(issue.id, null).then((updated) => {
                                    if (updated) {
                     ...
                     <CommandGroup heading="Assign to…">
                        {members.slice(0, 12).map((user) => (
                           <CommandItem
                              key={user.id}
                              onSelect={() => {
                                 void updateIssueAssignee(issue.id, user).then((updated) => {
                                    if (updated) {
