...
export function IssueContextMenu({ issueId }: IssueContextMenuProps) {
   ...
   const openDueDate = () => {
      // Quick edit means picking the date, not accepting a fixed offset.
      setDueDateValue(issue?.dueDate ? issue.dueDate.slice(0, 10) : '');
      setDueDateOpen(true);
   };

   const saveDueDate = async () => {
      if (!issueId || !dueDateValue) return;
   ...
   const clearDueDate = async () => {
      ...
      setDueDateSaving(true);
      try {
         if (await updateIssueDueDate(issueId, undefined)) {
   ...
   const handleRemindMe = () => {
      ...
      const date =
         existingReminder && !Number.isNaN(existingReminder.getTime())
            ? existingReminder
      ...
      <>
         <ContextMenuContent className="w-64">
            <ContextMenuGroup>
               ...
               <ContextMenuItem onClick={openDueDate}>
               ...
               <ContextMenuSub>
                  ...
                  <ContextMenuSubContent className="w-48">
                     ...
                     <ContextMenuItem
                        onClick={() => issueId && openIssueAction(issueId, 'convert-comment')}
                     >
         ...
         <Dialog open={dueDateOpen} onOpenChange={(open) => !dueDateSaving && setDueDateOpen(open)}>
            <DialogContent>
               <DialogHeader>
                  <DialogTitle>Set due date</DialogTitle>
                  <DialogDescription>The date this issue is expected to be done.</DialogDescription>
               </DialogHeader>
               <Input
                  type="date"
                  value={dueDateValue}
               ...
               <DialogFooter>
                  ...
                  <Button
                     ...
                     disabled={dueDateSaving || !dueDateValue}
                  >
         ...
         <Dialog
            open={reminderOpen}
            onOpenChange={(open) => !reminderSaving && setReminderOpen(open)}
         >
            <DialogContent>
               <DialogHeader>
                  <DialogTitle>Set reminder</DialogTitle>
                  <DialogDescription>
                     Receive an in-app notification at the selected time.
                  </DialogDescription>
               </DialogHeader>
               <Input
                  type="datetime-local"
