...
/** Team Overview retains the original two-column interface with API data. */
export default function TeamOverview() {
   const { orgId, teamId } = useParams<{ orgId: string; teamId: string }>();
   const { team, documents, loading, error } = useLiveTeam(teamId);

   if (loading)
      return <div className="px-8 py-10 text-sm text-muted-foreground">Loading team…</div>;
   if (error || !team)
      return (
   ...
   const goToLinks = [
      ...
      { label: 'Cycles', icon: RiDonutChartFill, href: `/${orgId}/team/${teamId}/cycles` },
      { label: 'Projects', icon: Box, href: `/${orgId}/team/${teamId}/projects` },
      { label: 'Views', icon: Layers, href: `/${orgId}/team/${teamId}/views` },
   ];
   ...
   return (
      <div className="w-full max-w-5xl mx-auto px-8 py-10 flex flex-col lg:flex-row gap-12">
         ...
         <div className="w-full lg:w-60 shrink-0">
            ...
            <Link
               href={`/${orgId}/team/${teamId}/members`}
