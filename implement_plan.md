...
| 2026-08-26 | P8y Agent and Reviews disabled | current change-set | Sidebar vẫn giữ Reviews/Agent đúng vị trí Circle nhưng hai mục bị disabled; loại bỏ review badge fixture. Command Palette không còn shortcut/route đến Agent hoặc Reviews, và mọi route trực tiếp hiển thị trạng thái Flowie unavailable thay vì UI mock. | Chỉ mở lại khi agent/review domain có backend thật và integration acceptance. | Rebuild web, hover/click Reviews/Agent ở sidebar (không điều hướng); thử URL trực tiếp và xác nhận màn unavailable. |
