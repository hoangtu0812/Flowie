...
| 2026-08-25 | P6p public Issue read contract | current change-set | FastAPI public `/issues` and `/issues/options` now serve the audited native list/options read contract; all Issue mutations remain private/legacy until individually covered. No frontend or UI file changed | Docker OpenAPI/API regression, then switch adapter URL with no presentation change | Verify public routes, then cut the list adapter from `_native` |
