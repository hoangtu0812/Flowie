...
| 2026-08-25 | P6p public Issue core contract | current change-set | FastAPI public `/issues`, `/issues/options`, `POST /issues`, `PATCH /issues/{id}` and `DELETE /issues/{id}` now expose native read/create/update/archive behavior. The Circle UI still uses the unchanged dialog/presentation until a data-only adapter can be verified; advanced mutations remain private/legacy | Docker OpenAPI/API regression, then data-only dialog adapter | Verify public routes, then migrate Create Issue submission |
