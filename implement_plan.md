...
| 2026-08-25 | P6o Circle Issue list adapter | current change-set | `issues-store` starts empty then maps authenticated Python Issue API data into the existing Circle presentation model; All Issues triggers load without markup/style changes; `npm run build` passed | Docker/browser acceptance and mutation adapters (create/edit/status/etc.) | Rebuild web only after 5G confirmation, then user verifies real Issue list |
