...
- API Jest: **12 suites, 28 tests passed** trong Docker build image, gồm validation Release và
  Customer Request mới.
- NestJS build: passed.
...
| P0 | Settings placeholders | `SLAs`, `Pulse`, `Asks`, `Emojis` vẫn là placeholder gốc và chưa có model/API. Phải thiết kế model + API + CRUD trong đúng khung UI gốc. `Releases` và `Customer requests` đã hoàn thành. |
...
2. Hoàn thiện một module placeholder theo lát dọc: ưu tiên `SLAs`, sau đó `Asks`, `Pulse`,
   `Emojis`; mỗi module gồm Prisma migration, Nest module/API, UI CRUD,
