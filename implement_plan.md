...
- API Jest: **14 suites, 32 tests passed** trong Docker image, gồm validation SLA và kiểm thử
  chọn policy theo độ cụ thể team/priority.
- NestJS build: passed.
...
| P0 | Settings placeholders | `Pulse`, `Asks`, `Emojis` vẫn là placeholder gốc và chưa có model/API. Phải thiết kế model + API + CRUD trong đúng khung UI gốc. `Releases`, `Customer requests` và `SLAs` đã hoàn thành. |
...
2. Hoàn thiện một module placeholder theo lát dọc: ưu tiên `Asks`, sau đó `Pulse`, `Emojis`;
   mỗi module gồm Prisma migration, Nest module/API, UI CRUD,
