...
| 2026-08-25 | P6p public Issue read/create contract | current change-set | FastAPI public `/issues` and `/issues/options` serve the audited native list/options contract; the Issue store uses these public Python routes instead of `_native`. Public `POST /issues` now exposes native Issue creation, while the Circle dialog has not yet been wired so no presentation changed; remaining mutations are still private/legacy | Docker OpenAPI/API regression, then data-only dialog adapter | Verify public routes, then migrate Create Issue submission |
