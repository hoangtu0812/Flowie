...
| 2026-08-25 | P6p public Issue read contract | current change-set | FastAPI public `/issues` and `/issues/options` now serve the audited native list/options read contract; the Issue store uses these public Python routes instead of `_native`; all Issue mutations remain private/legacy until individually covered. No JSX/style file changed | Docker OpenAPI/API regression | Verify public routes, then migrate the first mutation |
