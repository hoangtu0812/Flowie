...
| 2026-08-26 | P4h Native Markdown documents and folder management | current change-set | “Flowie document” được đổi thành Markdown document; tài liệu native mở editor web và auto-save title/content sau 800ms qua FastAPI. Folder có icon chọn/lưu thật, document có xóa soft-delete với xác nhận; header Documents dùng cùng grid cột với rows nên Created/Last edited thẳng hàng. Migration đổi record/default source type cũ `flowie` thành `markdown`; upload/link giữ nguyên. | Cần migration deploy + Docker/browser nghiệm thu editor/autosave/xóa | Tạo Markdown document, gõ nội dung, chờ Saved và refresh; đổi icon folder, xóa document, kiểm ngày Created/Last edited |
