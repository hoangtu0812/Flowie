...
| 2026-08-26 | P8r Documents & Initiatives Settings cutover | current change-set | Settings → Documents và Settings → Initiatives không còn render placeholder: dùng đúng Circle Settings shell cùng adapter dữ liệu FastAPI Python đã có list/create/edit/archive. Toàn bộ request đổi sang `authenticatedFetch`, do đó access cookie được làm mới khi chỉ còn durable refresh session. | Cần nghiệm thu trực tiếp với workspace thật; các trang Settings placeholder khác tiếp tục được xử lý theo từng domain backend. | Rebuild web, vào Settings → Documents/Initiatives, tạo/sửa/archive một bản ghi rồi refresh để xác nhận session và dữ liệu persisted. |
| 2026-08-26 | P8s Notification controls disabled | current change-set | Settings → Notifications giữ nguyên UI Circle nhưng toàn bộ kênh và preference switch được đặt disabled vì delivery chưa được triển khai đầy đủ. Nội dung LNDev UI đổi thành Flowie; bổ sung Discord với trạng thái cấu hình qua workspace integration, không mô phỏng trạng thái kết nối. | Cần kết nối delivery thật trước khi bật lại controls (ưu tiên Discord webhook đang có backend). | Rebuild web, mở Notifications và xác nhận không thể tương tác Desktop/Mobile/Email/Slack/Discord hoặc các switch; vào Workspace integration khi cần cấu hình Discord webhook. |
| 2026-08-26 | P8t Code & reviews controls disabled | current change-set | Giữ nguyên màn Settings → Code & reviews của Circle, đổi branding Flowie và khóa toàn bộ switch, select, signing-key action và coding-tools row. Không còn điều khiển giả cho GitHub/PR/code-review trong khi domain này chưa triển khai backend. | Cần Git provider + review domain thật trước khi bật lại từng control. | Rebuild web, mở Code & reviews, kiểm mọi control có thể xem nhưng không click/đổi giá trị được. |
| 2026-08-26 | P8u Security sessions live | current change-set | Settings → Security & access lấy session thật từ FastAPI Python `/auth/sessions`, hiển thị thiết bị/IP/last active và cho revoke toàn bộ session khác bằng persisted session store. Passkeys, API keys và commit signing key giữ UI nhưng disabled; toàn bộ branding cũ được thay Flowie. | Session activity phụ thuộc thiết bị thực đăng nhập; các security feature khác chỉ được bật khi backend tương ứng hoàn tất. | Rebuild web, đăng nhập từ hai browser/profile, mở Security kiểm current/other session rồi chọn Revoke all và xác nhận session còn lại bị đăng xuất. |
| 2026-08-26 | P8v Connected accounts Discord cutover | current change-set | Connected accounts đổi branding Flowie và khóa Slack/Google Calendar/Notion/GitHub/coding relay vì chưa có provider backend. Discord là integration duy nhất hoạt động: nút Connect mở form webhook thật, lưu/test qua API hiện hữu với durable authenticated session. | Discord backend đang đi qua legacy integration facade; cần port domain này sang Python native khi tiếp tục migration. | Rebuild web, vào Connected accounts → Discord → Connect, lưu webhook hợp lệ rồi Send test; xác nhận các integration khác không tương tác được. |
| 2026-08-26 | P8w Integrations catalog narrowed to Discord | current change-set | Settings → Integrations không còn hiển thị catalog/Enabled mock của GitHub, Slack, Figma, agents và các provider chưa làm. Màn chỉ giữ Discord ở vị trí đầu, cùng form webhook thật và Send test đang dùng ở Connected accounts. | Port Discord integration facade sang Python native và chỉ thêm provider mới khi có backend/acceptance tương ứng. | Rebuild web, mở Settings → Integrations; xác nhận chỉ Discord xuất hiện, Connect mở dialog và lưu/test webhook. |
| 2026-08-26 | P4n Settings team navigation live | current change-set | Khối “Your teams” trong Settings bỏ `mock-data/teams`, lấy đúng các Team mà người dùng đã tham gia trong workspace hiện tại và cập nhật qua event sau create/join. “Join or create a team” giờ tạo Team bằng FastAPI/PostgreSQL hoặc join Team thật theo join policy, thay vì form/list fixture. | Cần Docker/browser nghiệm thu create/join với workspace có Team thứ hai hoặc tài khoản khác. | Mở Settings, đối chiếu Your teams với Teams; chọn Join or create a team, tạo Team hoặc Join Team rồi refresh/sidebar phải cập nhật. |
| 2026-08-26 | P8x Agent personalization disabled | current change-set | Giữ màn Agent personalization theo UI Circle nhưng khóa Guidance, Skills và MCP connectors vì Flowie chưa có agent domain/backend tương ứng. Toàn bộ copy LNDev Agent đổi sang Flowie và trạng thái unavailable rõ ràng, không còn nút/ô nhập tạo mutation giả. | Bật dần từng control khi automation/agent backend có contract thật. | Rebuild web, mở Settings → Agent personalization và kiểm tra mọi control vẫn hiển thị nhưng không tương tác được. |
| 2026-08-27 | P8y Agent plan workflow; Reviews disabled | current change-set | Agent dùng provider OpenAI hoặc Google cấu hình theo workspace bởi Owner/Admin; key được mã hóa server-side. Conversation và proposal được lưu theo workspace/user; Markdown, DOCX, XLSX được trích nội dung tạm thời để tạo plan Project/Issue. LangGraph chỉ tạo proposal; nút Accept mới tạo record qua contract Project/Issue sẵn có. Reviews tiếp tục disabled. | Bổ sung kiểm thử integration với provider key thật, giới hạn usage/quota và approval/RBAC chi tiết trước khi mở action tự động. | Migrate DB; cấu hình Fernet key và một provider; gửi request/file, chỉnh proposal qua follow-up, Accept và refresh để xác nhận đúng Project/Issue được tạo một lần. |
| 2026-08-26 | P8z Workspace-aware navigation repair | current change-set | Sửa các đường dẫn runtime còn hard-code `/lndev-ui`: Inbox/My issues dùng slug workspace hiện tại, Project badge và Back to app cũng giữ đúng workspace. Vì vậy Inbox không còn gọi API với workspace cũ rồi báo “not available”. `flowie-icon.svg` được loại khỏi auth middleware để manifest nhận SVG thay vì HTML redirect. | Cần audit các route fixture còn lại theo từng module trước khi bật feature tương ứng. | Đăng nhập workspace thật, click Inbox/My issues/Project badge/Back to app; kiểm Network không có `/lndev-ui/...` và manifest icon không báo lỗi. |
| 2026-08-26 | P8za Web public assets runtime repair | current change-set | Docker web runtime copy `public/` vào cả process root và Next project root (`/app/apps/web/public`), đồng thời middleware bypass rõ ràng mọi static image. Manifest/static asset vì vậy trả MIME type thật (SVG/PNG) thay vì bị auth middleware đổi thành Login HTML, kể cả ở `/auth/login`. | Cần browser cache refresh sau deploy để manifest cũ được nạp lại. | Rebuild web, request `/flowie-icon.svg` phải là `image/svg+xml`; mở DevTools Console trên Login/Inbox không còn manifest image warning. |
| 2026-08-26 | P8zb Realtime Inbox & Discord delivery | current change-set | Python ghi notification bền vững trước, sau commit fan-out WebSocket `/api/v1/notifications/stream` đến đúng user/workspace; sidebar Inbox tự reconnect và hydrate lại từ REST. Issue create/assign/status, comment và project property/update đều phát sự kiện Inbox. Discord webhook workspace và bot broadcaster (token/channel qua server env) cùng nhận event, không làm mất Inbox khi Discord lỗi. Help menu được đổi link Flowie/GitHub; thêm `CHANGELOG.md`. | Bot hiện là outbound notifier; slash-command/interactions cần một scope riêng với Discord signature verification. Khi scale nhiều API replica, NotificationHub cần Redis pub/sub. | Rebuild API+web; mở hai session khác user, tạo/assign/đổi status/comment/project update và xác nhận badge Inbox tăng không refresh. Kết nối webhook hoặc bot và kiểm message Discord. |
